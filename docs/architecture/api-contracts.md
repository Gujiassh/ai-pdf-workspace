...
- HTTP 状态码为 `202 Accepted`，删除不是纯同步动作，因为需要清理 chunk 和对象存储文件
- 只有 workspace owner 可以发起删除，也只有 owner 可以对失败的 `delete_cleanup` 执行重试
...
### `POST /api/workspaces/:workspaceId/documents/:documentId/delete-retry`

作用：

- 对失败的 `delete_cleanup` 任务重新排队
- 保留原删除任务记录，按文档删除历史最大 `attemptCount + 1` 创建新任务
...
- `POST /v1/workspaces/{workspaceId}/documents/{documentId}/delete-retry`
