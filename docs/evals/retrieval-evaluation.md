...
```bash
cd /home/cc/code/ai-pdf-workspace/apps/api
...
- 纯中文问题使用 `pg_trgm` GiST KNN 词相似候选；先在 Workspace 边界内取候选，再按 ready、未删除、当前 index version 和 Document/Page workspace 条件严格复验。
- Dense 与 lexical 都按文档页去重后使用 `RRF(k=60)` 合并，避免同页多个 chunk 重复占用 citation 位置。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Dense | 0.7708 | 0.7229 | 0.6935 | 0.8500 | 7.3 ms | 85.6 ms |
| Hybrid/RRF | 0.8417 | 0.7354 | 0.7394 | 0.9250 | 22.0 ms | 109.9 ms |

`端到端` 口径为 query embedding + retrieval。Hybrid Recall 提升 `0.0708`，citation hit 提升 `0.0750`；端到端 `p95` 增加 `24.3 ms`，比例为 `1.284x`。4 worker 并发各执行 40 条：Dense/Hybrid 都是 `0` error、`0` result drift；Hybrid 并发 retrieval `p95=33.1 ms`、吞吐约 `136 req/s`。
