...
import type { CreateUploadSessionResponseDto } from "@/lib/assets/types";
...
export async function POST(
  ...
  const response = await fetch(`${getApiBaseUrl()}/v1/workspaces/${workspaceId}/assets/upload-session`, {
    method: "POST",
    cache: "no-store",
    headers: {
  ...
  return NextResponse.json({
    ...
    upload: {
      ...typed.upload,
      url: `/api/workspaces/${workspaceId}/assets/${typed.asset.id}/upload?objectKey=${encodeURIComponent(typed.upload.objectKey)}`,
    },
